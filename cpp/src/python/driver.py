...
#set vector-values
chparams.b = av.aligned_double_vector(1.0 * np.ones(nx**2))
chparams.u = av.aligned_double_vector(1.0 * np.ones(nx**2))
chparams.m = av.aligned_double_vector(0.0 * np.ones(nx**2))
chparams.T_const = av.aligned_double_vector(chparams.T_max * np.ones(nx**2))
...
for i in range(n_tsteps):
    info.t0          = t[i]
    info.tf          = t[i+1]
    chparams.T_const = av.aligned_double_vector(T[i] * np.ones(nx**2))
