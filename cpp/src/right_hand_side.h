...
public:
  virtual void rhs(const aligned_vector<real> &c, aligned_vector<real> &dcdt,
                   const real t) = 0;
  virtual void write_state( const aligned_vector<real> &x , const int idx , const int nx , const int ny , std::string& outdir) const = 0;
  virtual void setInitialConditions(aligned_vector<real> &x) const = 0;
